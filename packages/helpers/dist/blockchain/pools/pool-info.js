 ...
 */
async function getPoolInfo(poolId, network = 'mainnet') {
    try {
 ...
 */
async function getPoolsInfo(poolIds, network = 'mainnet') {
    const results = {};
 ...
 */
async function searchPools(query, network = 'mainnet') {
    try {
        ...
        // Search by ticker
        const tickerResponse = await fetch(`${apiUrl}?ticker=like.${query}*`, {
            method: 'GET',
            headers: {
        ...
        // Search by pool ID
        const idResponse = await fetch(`${apiUrl}?pool_id_bech32=like.${query}*`, {
            method: 'GET',
            headers: {
                'accept': 'application/json',
            }
        ...
        const uniquePoolIds = [...new Set(allPools
                .filter((pool) => pool && pool.pool_id_bech32)
                .map((pool) => pool.pool_id_bech32)
 ...
 */
function formatPoolInfo(poolInfo) {
    return {
 ...
 */
async function getPoolMetrics(poolId, network = 'mainnet') {
    try {
